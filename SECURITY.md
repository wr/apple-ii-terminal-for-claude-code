...
- **Retry limits.** The native client's first unrecognized token-shaped credential is treated as a stale disk token and does not spend a strike. After that, the first 3 wrong pairing-code guesses have no delay. Later misses sleep for 2 seconds, 4 seconds, then at most 8 seconds per attempt. The hard limit is 10 code guesses per source IP per bridge run. Strike counts survive reconnects during that run.
